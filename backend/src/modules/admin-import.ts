 ...
 */
adminImportRouter.post('/reimport-anatomie-pcem2-complete', requireAuth, async (req: any, res) => {
  try {
    // Vérifier que l'utilisateur est admin
    const user = await prisma.user.findUnique({
      where: { id: req.userId }
 ...
 * }
 */
adminImportRouter.post('/create-subject-complete', requireAuth, async (req: any, res) => {
  try {
    // Vérifier que l'utilisateur est admin
    const user = await prisma.user.findUnique({
      where: { id: req.userId }
