...
async function fixChapterTitlesFromFiles() {
    try {
        ...
        const anatomieDir = path_1.default.join(__dirname, '../data/quiz/pcem2/anatomie');
        const files = fs_1.default.readdirSync(anatomieDir).filter(f => f.endsWith('.txt'));
        // Créer un mapping fichier -> titre extrait
        const fileTitles = {};
        for (const file of files) {
            const filePath = path_1.default.join(anatomieDir, file);
        ...
        let fixedCount = 0;
        const usedTitles = new Set(); // Track already assigned titles
        const usedFiles = new Set(); // Track already used files
...
//# sourceMappingURL=fix-chapter-titles-from-files.js.map